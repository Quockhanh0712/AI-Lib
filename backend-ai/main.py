...
cache_version = 0
detector = MTCNN()
UPLOAD_DIR = "uploaded_photos" # <-- Đảm bảo nó nằm ở đây và KHÔNG BỊ LÙI VÀO (indent)
...
def extract_embedding(img):
    ...
    try:
        # Sử dụng ArcFace để trích xuất embedding
        # ArcFace thường không cần chuẩn hóa bên ngoài vì nó thường tạo ra các embedding đã được chuẩn hóa.
        # Tuy nhiên, DeepFace.represent() có thể trả về embedding đã được chuẩn hóa hoặc không,
        # và việc chuẩn hóa tường minh thêm một lần nữa sẽ không gây hại.
        result = DeepFace.represent(preprocessed_img, model_name="ArcFace", detector_backend="skip", enforce_detection=False)[0]
        tf.keras.backend.clear_session()
        embedding = np.array(result["embedding"])
        
        # Giữ lại logic chuẩn hóa để đảm bảo tính nhất quán nếu ArcFace của DeepFace
        # không luôn trả về embedding đã chuẩn hóa hoặc để tương thích với FaceNet trước đó.
...
# Giữ nguyên các hàm này vì chúng hoạt động hiệu quả trên numpy arrays
...
def process_frame(frame_data: str, db: Session):
    ...
    try:
        ...
        user_embeddings = load_embedding_cache(db) if embedding_cache is None else embedding_cache
        ...
        for user in user_embeddings:
            ...
            # Sử dụng các ngưỡng đã được cập nhật từ DeepFace
...
async def confirm_attendance(req: ConfirmAttendanceRequest, db: Session = Depends(get_db)):
    ...
    shift = "Sáng" if entry_time.hour < 12 else "Chiều" if entry_time.hour < 18 else "Tối"
...
@app.post('/add-user')
async def add_user(member_code: str = Form(...), full_name: str = Form(...), db: Session = Depends(get_db)):
    existing_user = db.query(User).filter(User.member_code == member_code).first()
    if existing_user:
        raise HTTPException(status_code=400, detail="Mã thành viên đã tồn tại")
    user = User(member_code=member_code, full_name=full_name)
    db.add(user)
    db.commit()
    return {"status": "Đã thêm", "id": user.id}
...
async def add_face(
    ...
    cache_version += 1
    embedding_cache = None
    return {"status": "Đã lưu embedding"}
