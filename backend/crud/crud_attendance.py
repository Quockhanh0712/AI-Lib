...
from db import models # Đảm bảo import models đúng
...
def checkout_attendance_session(db: Session, session_id: int, user_id_from_code: int) -> Optional[models.AttendanceSession]:
    ...
    if not db_session:
        return None # Bản ghi không tồn tại hoặc đã checkout

    # Kiểm tra xem user_id của bản ghi có khớp với user_id được xác minh qua member_code không
    ...
    if db_session.entry_time and db_session.exit_time:
        time_difference = db_session.exit_time - db_session.entry_time
