...
class UserProfileResponse(User):
    total_attendance_count: Optional[int] = None # MỚI: Thêm trường này
...
# Ví dụ:
class AttendanceSessionResponse(BaseModel): # Đã đổi tên từ CompletedAttendanceSession
    id: int
    user_id: int
    entry_time: str # Đã đổi thành str để khớp với định dạng từ backend
    exit_time: str # Đã đổi thành str để khớp với định dạng từ backend
